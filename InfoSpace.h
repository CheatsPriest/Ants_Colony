...
public:
	...
	//type - 1 = �������; under_class: 1 = Scout, 2 = Worker, 3 = Soldier, 0 = Queen
	...
	bool CreateEntity(int x, int y, int t); // ������� ������
	...
	void moveToCeilBackBase(pair<int, int> newPos2, unsigned int id, Insect* curr);
	...
	void spawnEat(pair<int, int> position) {
		...
		if ( ((double)(rand() % 10000) / 10000) <= chaince) {
			...
			CreateEntityFood(position.first, position.second, 0, 1, 4000, 10);
		}
	}
	void spawnEat1(pair<int, int> position) {
		double chaince = 0.001;
		if (((double)(rand() % 10000) / 10000) <= chaince) {
			CreateEntityFood(position.first+rand()%20-10, position.second+rand()%20-10, 0, 1, 4000, 10);
		}
	}
